...
class DinerTableViewController: UITableViewController,UISearchResultsUpdating, UISearchBarDelegate, UISearchControllerDelegate {
    //**Begin Copy**
    //2) Add filter search vars
    
    var filteredTableData = [NSManagedObject]()
    var resultSearchController = UISearchController()
    var dinerArray = [NSManagedObject]()
    ...
    func updateSearchResults(for searchController: UISearchController)
    {
        ...
        let searchPredicate = NSPredicate(format: "dinername CONTAINS[c] %@", searchController.searchBar.text!)
        let array = (dinerArray as NSArray).filtered(using: searchPredicate)
    ...
    func loaddb()
    {
        ...
        let fetchRequest:NSFetchRequest<NSFetchRequestResult> = NSFetchRequest(entityName: "Diner")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (self.resultSearchController.isActive) {
            ...
            cell?.textLabel?.text = person.value(forKey: "dinername") as! String?
            ...
            let cell =
                tableView.dequeueReusableCell(withIdentifier: "Cell")
                    as UITableViewCell?
            let person = dinerArray[(indexPath as NSIndexPath).row]
            cell?.textLabel?.text = person.value(forKey: "dinername") as! String?
            cell?.detailTextLabel?.text = ">>"
            return cell!
        }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete {
            ...
            if (self.resultSearchController.isActive) {
                ...
                context.delete(dinerArray[(indexPath as NSIndexPath).row])
            }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "UpdateDiners" {
            if let destination = segue.destination as?
                ...
                if (self.resultSearchController.isActive) {
                    if let SelectIndex = (tableView.indexPathForSelectedRow as NSIndexPath?)?.row {
                        ...
                        destination.dinerdb = selectedDevice
                        resultSearchController.isActive = false
                    }
                }
                else {
                    if let SelectIndex = (tableView.indexPathForSelectedRow as NSIndexPath?)?.row {
                        let selectedDevice:NSManagedObject = dinerArray[SelectIndex] as NSManagedObject
                        destination.dinerdb = selectedDevice
                    }
                }
            }
